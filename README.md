...
| `-ip[=<addr>]` | 输出 IP 列。bare `-ip`：被监控主机（本地 MySQL 显示本机网卡 IP，远程显示 `-H` 地址）；`-ip <addr>` 直接用指定地址 |

> **远程 MySQL 互斥**：`-H` 指向远程 MySQL 时，指定本地系统指标参数（`-l`/`-c`/`-s`/`-m`/`-d`/`-n`/`-sys`/`-lazy`）会报错——本地采集的 sys 指标与远程库指标混排会造成误导。仅本地 `-H`（`127.0.0.1`/`localhost`/本机 IP）允许混用。
